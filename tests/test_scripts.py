...
desired_seconds = desired_bars * (120/script_output_bpm) # 32
...
def preprocess(_in, _out, logfile=None, trim_start = -1):
    ...
    while best_length < target_seconds:
        ...
        if mid_out.length > best_length:
          best_length = mid_out.length
          best_length_start = trim_start
        
        if best_length >= target_seconds: break

        # If all file was consumed
        if trim_start + target_seconds > mid.length:
            write(f'No contiguous {target_seconds}s segment in {_out}')
            trim_start = best_length_start
            trim(trim_start)
            print(f'needs {target_seconds - best_length} seconds')
            runscript(prepscript(_out, _out, 'pad', '0', str(target_seconds - best_length)))
            break
    ...
    # TEST
    runscript(prepscript('-m', tmpfile, _out, 'trim', str(_start), str(target_seconds)))
